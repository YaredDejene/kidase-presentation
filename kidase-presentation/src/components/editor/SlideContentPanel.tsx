...
export const SlideContentPanel: React.FC<SlideContentPanelProps> = ({
  ...
  return (
    <div className="editor-content-panel">
      {slide.isDisabled && (
        <div className="editor-content-disabled-badge">DISABLED</div>
      )}

      {/* Title */}
      {slide.titleJson && (
        <div className="editor-content-section">
          <div className="editor-content-section-header">Title</div>
      ...
      <div className="editor-content-section">
        <div className="editor-content-section-header">
          Content{slide.blocksJson.length > 1 ? ` (${slide.blocksJson.length} blocks)` : ''}
        </div>
        {slide.blocksJson.map((block, blockIndex) => (
          <div key={blockIndex} className="editor-content-block">
            {slide.blocksJson.length > 1 && (
              <div className="editor-content-block-label">Block {blockIndex + 1}</div>
            )}
            {languages.map(({ slot, name }) => {
      ...
      {slide.footerJson && (
        <div className="editor-content-section">
          <div className="editor-content-section-header">Footer</div>
          {slide.footerJson.title && languages.map(({ slot, name }) => {
            ...
            return (
              <div key={`ft-${slot}`} className="editor-content-lang-row">
                <span className="editor-content-lang-label">{name} (title)</span>
                <span
                  className="editor-content-lang-value"
                  style={langColors[slot] ? { color: langColors[slot] } : undefined}
                >
          ...
          {slide.footerJson.text && languages.map(({ slot, name }) => {
            ...
            return (
              <div key={`fx-${slot}`} className="editor-content-lang-row">
                <span className="editor-content-lang-label">{name} (text)</span>
                <span
                  className="editor-content-lang-value"
                  style={langColors[slot] ? { color: langColors[slot] } : undefined}
                >
      ...
      {/* Details */}
      <div className="editor-content-section">
        <div className="editor-content-section-header">Details</div>
        ...
        {slide.isDynamic && (
          <div className="editor-detail-row">
            ...
            <span>Dynamic (verse-expanded at presentation time)</span>
          </div>
        )}
        {slide.notes && (
          <div className="editor-detail-row">
